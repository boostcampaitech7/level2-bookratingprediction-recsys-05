...
def train_log(args, model, dataloader, logger, setting):
    ...
    for epoch in range(args.train.epochs):
        ...
        for data in tqdm(dataloader['train_dataloader'], desc=f'[Epoch {epoch+1:02d}/{args.train.epochs:02d}]'):
            ...
            # 타겟 변수 로그 변환
            y_log = torch.log(y.float())  # 작은 값을 더해 로그 안정성 확보
            
            y_hat = model(x)
            loss = loss_fn(y_hat, y_log) # 변환된 y_log와 y_hat 사용
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if args.dataset.valid_ratio != 0:  # valid 데이터가 존재할 경우
            valid_loss, valid_metrics = valid_log(args, model, dataloader['valid_dataloader'], loss_fn)
...
def retrain_full_data(args, model, dataloader, loss_fn, best_epoch, setting):
    ...
    for epoch in range(best_epoch):
        ...
        for data in tqdm(full_train_loader, desc=f'[Retrain Epoch {epoch+1:02d}/{best_epoch:02d}]'):
            ...
            # 타겟 변수 로그 변환
            y_log = torch.log(y.float())
            
            y_hat = model(x)
            loss = loss_fn(y_hat, y_log)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
...
def valid_log(args, model, dataloader, loss_fn):
    model.eval()
    total_loss = 0
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            # 예측값과 실제값 역변환
            y_hat_exp = torch.exp(y_hat)
            y_exp = y.float()

    return total_loss / len(dataloader)


def test_log(args, model, dataloader, setting, checkpoint=None):
    predicts = list()
    if checkpoint:
    ...
    with torch.no_grad():
        for data in dataloader['test_dataloader']:
            x = get_input(args, data)
            y_hat = model(x)
            # 예측값 역변환
            y_hat_exp = torch.exp(y_hat)
            predicts.extend(y_hat_exp.tolist())
    return predicts
